...
首先，实现一个 `DemuxerConfig` 类用于定义音频解封装参数的配置。这里包括了：待解封装的资源、解封装类型这几个参数。这样设计是因为这个配置类不仅会用于音频解封装，后续的视频解封装也会使用。
...
接下来，我们实现一个 `MP4Demuxer` 类来实现 MP4 的解封装。它能从符合 MP4 标准的文件中解封装出音频编码数据。
...
上面是 `MP4Demuxer` 的接口设计，除了初始化方法，主要还有一些获取解封装器信息或者状态的属性接口，此外就是执行开始/取消读取数据、拷贝音频/视频采样数据的操作接口。

这里大家可能会疑惑，为什么 `MP4Demuxer` 不像前面的 Demo 中设计的 `AudioCapture`、`AudioEncoder` 的接口那样，有一个解封装后的数据回调接口。主要是因为解封装的速度是非常快的，不会成为一个音视频 pipeline 的瓶颈，而且考虑到解封装的资源可能会很大，所以一般不会一直不停地解出数据往外抛，这样下一个处理节点可能处理不过来这些数据。基于这些原因，解封装器的接口设计是让外部调用方主动找解封装器要数据来触发解封装操作，并且还要控制一定的缓存量防止内存占用过大。
...
#define MP4DemuxerBadFileError 2000
#define MP4DemuxerAddVideoOutputError 2001
#define MP4DemuxerAddAudioOutputError 2002
#define MP4DemuxerQueueMaxCount 3

@interface MP4Demuxer () {
    CMSimpleQueueRef _audioQueue;
    CMSimpleQueueRef _videoQueue;
}
@property (nonatomic, strong, readwrite) DemuxerConfig* config;
...
@property (nonatomic, assign, readwrite) MP4DemuxerStatus demuxerStatus; // 解封装器状态。
...
- (void)dealloc {
    // 清理状态机。
    if (self.demuxerStatus == MP4DemuxerStatusRunning) {
        self.demuxerStatus = MP4DemuxerStatusCancelled;
    }
...
- (void)startReading:(void (^)(BOOL success, NSError *error))completeHandler {
    ...
    dispatch_async(_demuxerQueue, ^{
        ...
        if (!weakSelf.demuxReader) {
            ...
            weakSelf.demuxerStatus = error ? MP4DemuxerStatusFailed : MP4DemuxerStatusRunning;
...
- (void)cancelReading {
    ...
    dispatch_async(_demuxerQueue, ^{
        ...
        weakSelf.demuxerStatus = MP4DemuxerStatusCancelled;
...
- (BOOL)hasAudioSampleBuffer {
    // 是否还有音频数据。
    if (self.hasAudioTrack && self.demuxerStatus == MP4DemuxerStatusRunning && !self.audioEOF) {
...
- (CMSampleBufferRef)copyNextAudioSampleBuffer CF_RETURNS_RETAINED {
    ...
    while (!sampleBuffer && self.demuxerStatus == MP4DemuxerStatusRunning && !self.audioEOF) {
        // 先从缓冲队列取数据。
        dispatch_semaphore_wait(_audioQueueSemaphore, DISPATCH_TIME_FOREVER);
        if (CMSimpleQueueGetCount(_audioQueue) > 0) {
        ...
        // 缓冲队列没有数据，就同步加载一下试试。
        if (!sampleBuffer && self.demuxerStatus == MP4DemuxerStatusRunning) {
...
- (BOOL)hasVideoSampleBuffer {
    // 是否还有视频数据。
    if (self.hasVideoTrack && self.demuxerStatus == MP4DemuxerStatusRunning && !self.videoEOF) {
...
- (CMSampleBufferRef)copyNextVideoSampleBuffer CF_RETURNS_RETAINED {
    // 拷贝下一份视频采样。
    CMSampleBufferRef sampleBuffer = NULL;
    while (!sampleBuffer && self.demuxerStatus == MP4DemuxerStatusRunning && !self.videoEOF) {
        // 先从缓冲队列取数据。
        dispatch_semaphore_wait(_videoQueueSemaphore, DISPATCH_TIME_FOREVER);
        if (CMSimpleQueueGetCount(_videoQueue) > 0) {
...
- (void)_setupDemuxReader:(NSError**)error {
    ...
    // 3、处理待解封装的资源中的视频。
    if (self.config.demuxerType & MediaVideo) {
        // 取出视频轨道。
        ...
        if (_hasVideoTrack) {
            ...
            if ([_demuxReader canAddOutput:_readerVideoOutput]) {
                ...
                *error = _demuxReader.error ? _demuxReader.error : [NSError errorWithDomain:NSStringFromClass([self class]) code:MP4DemuxerAddVideoOutputError userInfo:nil];
                return;
            }
        }
    }
    ...
    // 4、处理待解封装的资源中的音频。
    if (self.config.demuxerType & MediaAudio) {
        // 取出音频轨道。
        ...
        if (_hasAudioTrack) {
            ...
            if ([_demuxReader canAddOutput:_readerAudioOutput]) {
                ...
                *error = _demuxReader.error ? _demuxReader.error : [NSError errorWithDomain:NSStringFromClass([self class]) code:MP4DemuxerAddAudioOutputError userInfo:nil];
                return;
            }
        }
    }
    ...
    if (!_hasVideoTrack && !_hasAudioTrack) {
        *error = [NSError errorWithDomain:NSStringFromClass([self class]) code:MP4DemuxerBadFileError userInfo:nil];
        return;
    }
...
- (void)_loadNextSampleBuffer {
    ...
    if (self.demuxReader.status == AVAssetWriterStatusCompleted) {
        self.demuxerStatus = MP4DemuxerStatusCompleted;
        return;
        ...
        if (self.demuxReader.status == AVAssetWriterStatusFailed) {
            // 如果状态依然是失败，就上报错误。
            self.demuxerStatus = MP4DemuxerStatusFailed;
    ...
    BOOL audioNeedLoad = (self.config.demuxerType & MediaAudio) && !self.audioEOF;
    BOOL videoNeedLoad = (self.config.demuxerType & MediaVideo) && !self.videoEOF;
    while (self.demuxReader && self.demuxReader.status == AVAssetReaderStatusReading && (audioNeedLoad || videoNeedLoad)) {
        ...
        if (audioNeedLoad) {
            ...
            dispatch_semaphore_signal(_audioQueueSemaphore);
            if (audioCount < MP4DemuxerQueueMaxCount) {
                // 从音频输出源读取音频数据。
        ...
        if (videoNeedLoad) {
            ...
            dispatch_semaphore_signal(_videoQueueSemaphore);
            if (videoCount < MP4DemuxerQueueMaxCount) {
                // 从视频输出源读取视频数据。
...
上面是 `MP4Demuxer` 的实现，从代码上可以看到主要有这几个部分：
...
- - 这两个队列分别是 `_audioQueue` 和 `_videoQueue`。
  - 当外部向解封装器要数据而触发数据加载时，会把解封装后的数据先缓存到这两个队列中，缓冲的采样数不超过 `MP4DemuxerQueueMaxCount`，以减少内存占用。
...
- - 在枚举 `MP4DemuxerStatus` 中定义了解封装器的各种状态，对于解封装器的状态机管理贯穿在解封装的整个过程中。
...
上面是 `AudioDemuxerViewController` 的实现，其中主要包含这几个部分：
...
- - 在 `-fetchAndSaveDemuxedData` → `-saveSampleBuffer` 中实现。
  - 需要注意的是，我们从解封装器读取的音频 AAC 编码数据在存储为 AAC 文件时需要条件 ADTS 头。生成一个 AAC packet 对应的 ADTS 头数据在 `AudioTools` 类的工具方法 `+adtsDataWithChannels:sampleRate:rawDataLength:` 中实现。这个在前面的音频编码的 Demo 中已经介绍过了。
