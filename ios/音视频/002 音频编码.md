...
在这个 Demo 中，音频采集模块 `AudioCapture` 的实现与 iOS 音频采集中一样，这里就不再重复介绍了，其接口如下：
...
接下来，我们来实现一个音频编码模块 `AudioEncoder`，在这里输入采集后的数据，输出编码后的数据。
...
上面是 `AudioEncoder` 接口的设计，除了`初始化方法`，主要是有`获取音频编码码率`以及音频编码`数据回调`和`错误回调`的接口，另外就是`编码`的接口。
...
上面是 `AudioEncoder` 的实现，从代码上可以看到主要有这几个部分：
...
- 2）实现音频编码逻辑，并在将数据封装到 `CMSampleBufferRef` 结构中，抛给 AudioEncoder 的对外数据回调接口。
...
- 3）捕捉音频编码过程中的错误，抛给 AudioEncoder 的对外错误回调接口。
...
```
#import "AudioEncoderViewController.h"
#import <AVFoundation/AVFoundation.h>
#import "AudioCapture.h"
#import "AudioEncoder.h"
#import "AudioTools.h"

@interface AudioEncoderViewController ()
...
@implementation AudioEncoderViewController
    ...
    if (!_audioEncoder) {
        ...
        _audioEncoder.sampleBufferOutputCallBack = ^(CMSampleBufferRef sampleBuffer) {
            if (sampleBuffer) {
                ...
                [weakSelf.fileHandle writeData:[AudioTools adtsDataWithChannels:audioFormat.mChannelsPerFrame sampleRate:audioFormat.mSampleRate rawDataLength:totolLength]];
...
上面是 `AudioEncoderViewController` 的实现，其中主要包含这几个部分：
...
- 4）在编码模块 `AudioEncoder` 的数据回调中获取编码后的 AAC 裸流数据，并在每个 AAC packet 前写入 ADTS 头数据，存储到文件中。

- - 在 `AudioEncoder` 的 `sampleBufferOutputCallBack` 回调中实现。
  - 其中生成一个 AAC packet 对应的 ADTS 头数据在 `AudioTools` 类的工具方法 `+adtsDataWithChannels:sampleRate:rawDataLength:` 中实现。
