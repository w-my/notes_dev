...
首先，实现一个 `AudioConfig` 类用于定义音频采集参数的配置。这里包括了：采样率、量化位深、声道数这几个参数。
...
接下来，我们实现一个 `AudioCapture` 类来实现音频采集。
...
上面是 `AudioCapture` 的接口设计，可以看到这里除了`初始化方法`，主要是有`获取音频配置`以及音频采集`数据回调`和`错误回调`的接口，另外就是`开始采集`和`停止采集`的接口。
...
@property (nonatomic, strong, readwrite) AudioConfig *config;
...
- (void)startRunning {
    ...
    dispatch_async(_captureQueue, ^{
        ...
        if (startStatus != noErr) {
            // 捕捉并回调开始采集时的错误。
            [weakSelf callBackError:[NSError errorWithDomain:NSStringFromClass([AudioCapture class]) code:startStatus userInfo:nil]];
        }
    });
}
...
- (void)stopRunning {
    ...
    dispatch_async(_captureQueue, ^{
        if (weakSelf.audioCaptureInstance) {
            ...
            if (stopStatus != noErr) {
                // 捕捉并回调停止采集时的错误。
                [weakSelf callBackError:[NSError errorWithDomain:NSStringFromClass([AudioCapture class]) code:stopStatus userInfo:nil]];
            }
        }
    });
}
...
static OSStatus audioBufferCallBack(void *inRefCon,
    ...
    @autoreleasepool {
        ...
        if (status == noErr) {
            ...
            CMSampleBufferRef sampleBuffer = [AudioCapture sampleBufferFromAudioBufferList:buffers inTimeStamp:inTimeStamp inNumberFrames:inNumberFrames description:capture.audioFormat];
            // 回调数据。
...
上面是 `AudioCapture` 的实现，从代码上可以看到主要有这几个部分：

- 1）创建音频采集实例。第一次调用 `-startRunning` 才会创建音频采集实例。

- - 在 `-setupAudioCaptureInstance:` 方法中实现。

- 2）处理音频采集实例的数据回调，并在回调中将数据封装到 `CMSampleBufferRef` 结构中，抛给 AudioCapture 的对外数据回调接口。
...
- 4）捕捉音频采集开始和停止操作中的错误，抛给 AudioCapture 的对外错误回调接口。
...
上面是 `AudioCaptureViewController` 的实现，这里需要注意的是在采集音频前需要设置 **AVAudioSession**[6] 为正确的采集模式。
