...
首先，实现一个 `VideoCaptureConfig` 类用于定义视频采集参数的配置。
...
上面我们在 `VideoCaptureConfig` 的初始化方法里提供了一些默认值。

接下来，我们实现一个 `VideoCapture` 类来实现视频采集。
...
上面是 `VideoCapture` 的接口设计，可以看到这些接口类似音频采集器的接口设计，除了`初始化方法`，主要是有`获取视频配置`以及视频采集`数据回调`和`错误回调`的接口，另外就是`开始采集`和`停止采集`的接口。
...
- (AVCaptureSession *)captureSession {
    if (!_captureSession) {
        ...
        // 6、更新采集实时帧率。
        [self.captureDevice locorConfiguration:nil];
        [self _updateActiveFrameDuration];
        [self.captureDevice unlocorConfiguration];
...
上面是 `VideoCapture` 的实现，结合下面这两张图可以让我们更好地理解这些代码：

AVCaptureSession 配置多组输入输出

AVCaptureConnection 连接单或多输入到单输出
...
上面是 `VideoCaptureViewController` 的实现，主要分为以下几个部分：
