...
在实现视频编码模块之前，我们先实现一个视频编码配置类 `VideoEncoderConfig`：
...
接下来，我们来实现一个视频编码模块 `VideoEncoder`，在这里输入采集后的数据，输出编码后的数据。
...
上面是 `VideoEncoder` 接口的设计，除了`初始化方法`，主要是有`获取视频编码配置`以及视频编码`数据回调`和`错误回调`的接口，另外就是`编码`、`刷新重建编码器`、`清空编码缓冲区`的接口。
...
#define EncoderRetrySessionMaxCount 5
#define EncoderEncodeFrameFailedMaxCount 20

@interface VideoEncoder ()
@property (nonatomic, assign) VTCompressionSessionRef compressionSession;
@property (nonatomic, strong, readwrite) VideoEncoderConfig *config; // 视频编码配置参数。
...
- (void)encodePixelBuffer:(CVPixelBufferRef)pixelBuffer ptsTime:(CMTime)timeStamp {
    // 编码。
    if (!pixelBuffer || self.retrySessionCount >= EncoderRetrySessionMaxCount || self.encodeFrameFailedCount >= EncoderEncodeFrameFailedMaxCount) {
        return;
    }
    ...
    dispatch_async(self.encoderQueue, ^{
        ...
        if (!weakSelf.compressionSession || weakSelf.needRefreshSession) {
            ...
            if (setupStatus != noErr) {
                [weakSelf _releaseCompressionSession];
                NSLog(@"VideoEncoder setupCompressionSession error:%d", setupStatus);
            } else {
                weakSelf.needRefreshSession = NO;
            }
        }
        
        // 重试超过 EncoderRetrySessionMaxCount 次仍然失败则认为创建失败，报错。
        if (!weakSelf.compressionSession) {
            CFRelease(pixelBuffer);
            dispatch_semaphore_signal(weakSelf.semaphore);
            if (weakSelf.retrySessionCount >= EncoderRetrySessionMaxCount && weakSelf.errorCallBack) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    weakSelf.errorCallBack([NSError errorWithDomain:NSStringFromClass([VideoEncoder class]) code:setupStatus userInfo:nil]);
                });
            }
            return;
        }
        ...
        if (encodeStatus == kVTInvalidSessionErr) {
            ...
            NSLog(@"VideoEncoder kVTInvalidSessionErr");
        }
        // 记录编码失败次数。
        if (encodeStatus != noErr) {
            NSLog(@"VideoEncoder VTCompressionSessionEncodeFrame error:%d", encodeStatus);
        }
        ...
        // 编码失败次数超过 EncoderEncodeFrameFailedMaxCount 次，报错。
        if (weakSelf.encodeFrameFailedCount >= EncoderEncodeFrameFailedMaxCount && weakSelf.errorCallBack) {
            dispatch_async(dispatch_get_main_queue(), ^{
                weakSelf.errorCallBack([NSError errorWithDomain:NSStringFromClass([VideoEncoder class]) code:encodeStatus userInfo:nil]);
            });
        }
    });
}
...
上面是 `VideoEncoder` 的实现，从代码上可以看到主要有这几个部分：
...
`VideoEncoderViewController.m`

```
#import "VideoEncoderViewController.h"
#import "VideoCapture.h"
#import "VideoEncoder.h"

@interface VideoPacketExtraData : NSObject
@property (nonatomic, strong) NSData *sps;
@property (nonatomic, strong) NSData *pps;
@property (nonatomic, strong) NSData *vps;
@end

@implementation VideoPacketExtraData
@end


@interface VideoEncoderViewController ()
...
@implementation VideoEncoderViewController
...
- (VideoPacketExtraData *)getPacketExtraData:(CMSampleBufferRef)sampleBuffer {
    ...
    VideoPacketExtraData *extraData = nil;
    if (codecType == kCMVideoCodecType_H264) {
        ...
        if (statusCode == noErr) {
            ...
            if (statusCode == noErr) {
                extraData = [[VideoPacketExtraData alloc] init];
...
- (void)saveSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    if (sampleBuffer) {
        ...
        if ([self isKeyFrame:sampleBuffer]) {
            VideoPacketExtraData *extraData = [self getPacketExtraData:sampleBuffer];
            if (extraData.vps) {
...
上面是 `VideoEncoderViewController` 的实现，主要分为以下几个部分：
