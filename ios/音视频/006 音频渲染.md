...
同样的，解封装模块 `AudioDecoder` 的实现与 [《iOS 音频解码 Demo》](https://mp.weixin.qq.com/s?__biz=MjM5MTkxOTQyMQ==&mid=2257484944&idx=1&sn=63616655888d93557f935bc12088873e&scene=21#wechat_redirect) 中一样，这里就不再重复介绍了，其接口如下：
...
接下来，我们来实现一个音频渲染模块 `AudioRender`，在这里输入解码后的数据进行渲染播放。
...
上面是 `AudioRender` 接口的设计，除了`初始化`接口，主要是有音频渲染`数据输入回调`和`错误回调`的接口，另外就是`获取声道数`和`获取采样率`的接口，以及`开始渲染`和`结束渲染`的接口。

这里重点需要看一下音频渲染`数据输入回调`接口，系统的音频渲染单元每次会主动通过回调的方式要数据，我们这里封装的 `AudioRender` 则是用`数据输入回调`接口来从外部获取一组待渲染的音频数据送给系统的音频渲染单元。
...
- (void)startPlaying {
    ...
    dispatch_async(_renderQueue, ^{
        ...
        if (status != noErr) {
            // 捕捉并回调开始渲染时的错误。
            [weakSelf _callBackError:[NSError errorWithDomain:NSStringFromClass([AudioRender class]) code:status userInfo:nil]];
        }
    });
}
...
static OSStatus audioRenderCallback(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inOutputBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
    ...
    AudioRender *audioRender = (__bridge AudioRender *) inRefCon;
...
上面是 `AudioRender` 的实现，从代码上可以看到主要有这几个部分：

- 1）创建音频渲染实例。第一次调用 `-startPlaying` 才会创建音频渲染实例。

- - 在 `-_setupAudioRenderInstance:` 方法中实现。

- 2）处理音频渲染实例的数据回调，并在回调中通过 AudioRender 的对外数据输入回调接口向更外层要待渲染的数据。
...
- 4）捕捉音频渲染开始和停止操作中的错误，抛给 AudioRender 的对外错误回调接口。
...
`AudioRenderViewController.m`

```
#import "AudioRenderViewController.h"
#import <AVFoundation/AVFoundation.h>
#import "AudioRender.h"
#import "MP4Demuxer.h"
...
#define DecoderMaxCache 4096 * 5 // 解码数据缓冲区最大长度。

@interface AudioRenderViewController ()
...
@implementation AudioRenderViewController
...
- (AudioRender *)audioRender {
    if (!_audioRender) {
        ...
        _audioRender = [[AudioRender alloc] initWithChannels:1 bitDepth:16 sampleRate:44100];
        _audioRender.errorCallBack = ^(NSError* error) {
            NSLog(@"AudioRender error:%zi %@", error.code, error.localizedDescription);
        };
...
- (void)viewDidLoad {
    ...
    _timer = [CADisplayLink displayLinkWithTarget:[WeakProxy proxyWithTarget:self] selector:@selector(timerCallBack:)];
    [_timer addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
    [_timer setPaused:NO];
    
    [self.demuxer startReading:^(BOOL success, NSError * _Nonnull error) {
        NSLog(@"MP4Demuxer start:%d", success);
    }];
}
...
- (void)timerCallBack:(CADisplayLink *)link {
    // 定时从文件中解封装和解码一定量（不超过 DecoderMaxCache）的数据。
    if (self.pcmDataCacheLength <  DecoderMaxCache && self.demuxer.demuxerStatus == MP4DemuxerStatusRunning && self.demuxer.hasAudioSampleBuffer) {
...
上面是 `AudioRenderViewController` 的实现，其中主要包含这几个部分：

- 1）在页面加载完成后就启动解封装和解码模块，并通过一个 timer 来驱动解封装器和解码器。

- - 在 `-viewDidLoad` 中实现。

- 2）定时从文件中解封装一定量（不超过 DecoderMaxCache）的数据送给解码器。
...
- 4）在解码模块 `AudioDecoder` 的数据回调中获取解码后的 PCM 数据缓冲起来等待渲染。

- - 在 `AudioDecoder` 的 `sampleBufferOutputCallBack` 回调中实现。

- 5）在渲染模块 `AudioRender` 的输入数据回调中把缓冲区的数据交给系统音频渲染单元渲染。

- - 在 `AudioRender` 的 `audioBufferInputCallBack` 回调中实现。
