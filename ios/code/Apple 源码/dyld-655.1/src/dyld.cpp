...
static void addImage(ImageLoader* image)
{
	// add to master list
    allImagesLock();
	// 添加到 static std::vector<ImageLoader*> sAllImages
...
// 通过 header 里的 magic , cputype , cpusubtype 去检测是否兼容
...
// 我们需要为已经映射到主可执行文件中的文件制作一个ImageLoader*
// The kernel maps in main executable before dyld gets control.  We need to 
// make an ImageLoader* for the already mapped in main executable.
static ImageLoaderMachO* instantiateFromLoadedImage(const macho_header* mh, uintptr_t slide, const char* path)
{
	// try mach-o loader
	// isCompatibleMachO 通过 header 里的 magic , cputype , cpusubtype 去检测是否兼容
	if ( isCompatibleMachO((const uint8_t*)mh, path) ) {
		// 验证通过
		// instantiateMainExecutable 实例化这个 image ,
		// 并添加到 static std::vector<ImageLoader*> sAllImages; 这个全局的镜像列表中去 , 设置好上下文
...
//
// dyld 的入口点，内核加载dyld 并跳转到 __dyld_start ，设置一些寄存器并调用这个函数
// 返回目标程序中_dyld_start跳转到的main（）的地址
// uintptr_t/intptr_t 类型是为指针准备的,用来存放指针地址，
// 提供了一种可移植且安全的方法声明指针，而且和系统中使用的指针长度相同，对于把指针转化成整数形式来说很有用
// 加载app 的主要函数

// 1、配置相关环境变量
// 2、设置上下文信息 setContext
// 3、检测进程是否受限，在上下文中做出对应的处理 configureProcessRestrictions，检查环境变量 checkEnvironmentVariables
// 4、根据环境变量配置打印信息 , DYLD_PRINT_OPTS 与 DYLD_PRINT_ENV
// 5、获取程序架构 getHostInfo
uintptr_t
