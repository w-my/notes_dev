...
///
/// 1、首先 , 根据计算出来的 ASLR 的 slide 来重定向 macho .
/// 2、初始化 , 允许 dyld 使用 mach 消息传递 .
/// 3、栈溢出保护 .
/// 4、初始化完成后调用 dyld 的 main 函数 ,dyld::_main .
				...
				uintptr_t* startGlue)
{
    ...
    // 首先根据计算出来的ASLR的slide 来重定向 mach-O
    // 物理地址 = ASLR + 虚拟地址(偏移地址)
