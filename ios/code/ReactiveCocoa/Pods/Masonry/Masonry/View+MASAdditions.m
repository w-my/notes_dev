...
//返回添加的所有约束 MASCompositeConstraint
- (NSArray *)mas_makeConstraints:(void(^)(MASConstraintMaker *))block {
   ...
   //实例化constraintMaker对象，来操作接下来的约束
    ...
    //block作为参数，这里完成隐式调用
    //通过block将constraintMaker对象回调给用户让用户对constraintMaker中的MAConstraint类型的属性进行初始化。
    //换句话说block中所做的事情就是之前用户设置约束是所添加的代码，比如make.top(@10) ==> ( constraintMaker.top = 10 )。
    block(constraintMaker);
    //MASCompositeConstraint
    //添加约束 并返回所install 的约束数组 (NSArray<MASConstraint>)
