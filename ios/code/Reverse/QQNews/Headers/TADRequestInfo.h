//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class NSArray, NSDictionary, NSString, TADBreakPoint;

@interface TADRequestInfo : NSObject
{
    long long _adType;
    NSString *_videoId;
    NSString *_coverId;
    NSString *_lid;
    NSString *_pid;
    NSString *_fmt;
    NSString *_live;
    NSString *_dtype;
    NSString *_playmode;
    NSString *_tpid;
    NSString *_appId;
    NSString *_vip;
    NSString *_uin;
    NSString *_skey;
    NSArray *_cookieInfo;
    NSString *_guid;
    NSDictionary *_more;
    NSDictionary *_adParamMap;
    NSDictionary *_pageInfo;
    TADBreakPoint *_breakPoint;
}

@property(retain, nonatomic) TADBreakPoint *breakPoint; // @synthesize breakPoint=_breakPoint;
@property(retain, nonatomic) NSDictionary *pageInfo; // @synthesize pageInfo=_pageInfo;
@property(retain, nonatomic) NSDictionary *adParamMap; // @synthesize adParamMap=_adParamMap;
@property(retain, nonatomic) NSDictionary *more; // @synthesize more=_more;
@property(retain, nonatomic) NSString *guid; // @synthesize guid=_guid;
@property(retain, nonatomic) NSArray *cookieInfo; // @synthesize cookieInfo=_cookieInfo;
@property(retain, nonatomic) NSString *skey; // @synthesize skey=_skey;
@property(retain, nonatomic) NSString *uin; // @synthesize uin=_uin;
@property(retain, nonatomic) NSString *vip; // @synthesize vip=_vip;
@property(retain, nonatomic) NSString *appId; // @synthesize appId=_appId;
@property(retain, nonatomic) NSString *tpid; // @synthesize tpid=_tpid;
@property(retain, nonatomic) NSString *playmode; // @synthesize playmode=_playmode;
@property(retain, nonatomic) NSString *dtype; // @synthesize dtype=_dtype;
@property(retain, nonatomic) NSString *live; // @synthesize live=_live;
@property(retain, nonatomic) NSString *fmt; // @synthesize fmt=_fmt;
@property(retain, nonatomic) NSString *pid; // @synthesize pid=_pid;
@property(retain, nonatomic) NSString *lid; // @synthesize lid=_lid;
@property(retain, nonatomic) NSString *coverId; // @synthesize coverId=_coverId;
@property(retain, nonatomic) NSString *videoId; // @synthesize videoId=_videoId;
@property(nonatomic) long long adType; // @synthesize adType=_adType;
- (void).cxx_destruct;
- (id)getUserInfo;
- (id)getVideoInfo;

@end

