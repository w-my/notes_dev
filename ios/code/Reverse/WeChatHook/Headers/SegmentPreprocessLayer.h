//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "CNNKernel.h"

@class CNNBuffer;

@interface SegmentPreprocessLayer : CNNKernel
{
    CNNBuffer *_dimensionsBuffer;
    CNNBuffer *_paraBuffer;
    CNNBuffer *_scaleXBuffer;
    CNNBuffer *_scaleYBuffer;
    struct SegmentPreprocessPara _para;
}

@property(retain, nonatomic) CNNBuffer *scaleYBuffer; // @synthesize scaleYBuffer=_scaleYBuffer;
@property(retain, nonatomic) CNNBuffer *scaleXBuffer; // @synthesize scaleXBuffer=_scaleXBuffer;
@property(retain, nonatomic) CNNBuffer *paraBuffer; // @synthesize paraBuffer=_paraBuffer;
@property(retain, nonatomic) CNNBuffer *dimensionsBuffer; // @synthesize dimensionsBuffer=_dimensionsBuffer;
@property(nonatomic) struct SegmentPreprocessPara para; // @synthesize para=_para;
- (id).cxx_construct;
- (void).cxx_destruct;
- (void)findBestThreadgroupsPerGrid:(CDStruct_14f26992 *)arg1 threadsPerThreadgroup:(CDStruct_14f26992 *)arg2;
- (_Bool)encodeWithCommandBuffer:(id)arg1 sourceImage:(id)arg2 destinationImage:(id)arg3;
- (id)constructWithLayerData:(const struct layer_res *)arg1;
- (void)preloadWithInputDimension:(struct CNNDimension)arg1;
- (void)preloadWithPara:(struct SegmentPreprocessPara)arg1 inputDimension:(struct CNNDimension)arg2;
- (id)initWithPara:(struct SegmentPreprocessPara)arg1 inputDimension:(struct CNNDimension)arg2 device:(id)arg3 library:(id)arg4;

@end

