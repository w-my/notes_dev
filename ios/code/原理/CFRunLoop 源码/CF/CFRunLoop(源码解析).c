...
#pragma mark 获取/创建 runLoop
/**
 * 从下面的源码可以看出 线程和 RunLoop 之间是一一对应的
 * 其关系是保存在一个全局的 Dictionary 里
...
// (pthread_t)0 代表主线程
// 获取一个 pthread_t 对应的 RunLoop
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    // kNilPthreadT = 0
    // 首先确保开启主线程  pthread_main_thread_np() 用来获取主线程
    if (pthread_equal(t, (pthread_t)0)) {
        t = pthread_main_thread_np();  //主线程
    }
    
    __CFSpinLock(&loopsLock);
    
    //第一次进入字典为空时 初始化全局 dict 并先为主线程创建一个 RunLoop
    if (!__CFRunLoops) {
    ...
    CFRunLoopRef mainLoop = __CFRunLoopCreate(pthread_main_thread_np());
        // dict : dict
        //key:pthreadPointer(pthread_main_thread_np())
        //value:mainLoop
    ...
    if (pthread_equal(t, pthread_self())) {  //子线程
        //注册一个回调 ，当前线程销毁时，顺便也销毁对应的 RunLoop
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    do {
        ...
        //2、通知 Observers : RunLoop 即将 处理 Timers
         __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeTimers);
        //3、通知 Observers : RunLoop 即将 处理 Source0 (非port)
         __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeSources);

        //4、处理Blocks
        __CFRunLoopDoBlocks(rl, rlm);
      
        //5、处理 Sources0（非 port）
        ...
        //6、判读有无 Source1 (基于port) 处于 ready 状态
        //  有跳转到 handle_msg 处理这个 Source1
        if (__CFRunLoopServiceMachPort(dispatchPort, &msg, sizeof(msg_buffer), &livePort, 0)) {
            //如果有 Source1 跳转到 handle_msg
            goto handle_msg;
        }
        
        //7、通知 Observers: 即将休眠 (sleep)
	__CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeWaiting);
	__CFRunLoopSetSleeping(rl);
	// do not do any user callouts after this point (after notifying of sleeping)
     
        // 调用 mach_msg 等待接受 mach_port 的消息
        // 线程将进入休眠,直到被以下的某个线程唤醒
        // 1、一个基于 port 的 Source 事件
        // 2、一个 Timer 到时间了
        // 3、RunLoop 自身的超时时间到了
        // 4、被其他的调用者手动唤醒等
        
        // 等待别的消息来唤醒当前线程 （线程阻塞）
        // 休眠的实现原理是： 从用户态切换到内核态， 然后等待消息 没有消息就让线程休眠，有消息就唤醒线程
        // 阻塞就是从用户态 切换到 内核态
        // 内核处理 是真正的休眠 不是简单的 while 死循环 (内核层面的阻塞)
        __CFRunLoopServiceMachPort(waitSet, &msg, sizeof(msg_buffer), &livePort, poll ? 0 : TIMEOUT_INFINITY);
        
	__CFRunLoopUnsetSleeping(rl);
        //8、通知 Observer: 被唤醒了，结束休眠
	 __CFRunLoopDoObservers(rl, rlm, kCFRunLoopAfterWaiting);

    // 收到消息 处理消息
    handle_msg:
        //8.1 如果一个 Timer 到时间了，触发这个Timer的回调
        if (如果被 timer 唤醒）
            //处理 Timers
            __CFRunLoopDoTimers(rl, rlm, mach_absolute_time());
            //8.2 如果有dispatch 到 main_queue 的block，执行block。
        else if (被gcd唤醒）
            __CFRUNLOOP_IS_SERVICING_THE_MAIN_DISPATCH_QUEUE__(msg);
        } else { //8.3 如果一个 Source1 (基于port) 发出事件了，处理这个事件
            //处理 Source1
            __CFRunLoopDoSource1(rl, rlm, rls, msg, msg->msgh_size, &reply) || sourceHandledThisLoop;
        
        //9、处理加入到 Loop 的 Blcoks
        __CFRunLoopDoBlocks(rl, rlm);
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds,
    ...
    // 11、通知 Observer 退出 RunLoop
	if (currentMode->_observerMask & kCFRunLoopExit ) __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopExit);

    return result;
}

#pragma mark RunLoop 的主函数，是一个死循环
// 用DefaultMode启动
// RunLoop 的主函数，是一个死循环
void CFRunLoopRun(void) {	/* DOES CALLOUT */
    int32_t result;
    do {
        // CFRunLoopRunSpecific 具体处理runloop 的运行情况
        // CFRunLoopGetCurrent() 当前runloop 对象
        // kCFRunLoopDefaultMode runloop 的运行模式
        // 1.0e10  runloop 的默认运行时间，即超时为 1.0e10 (10的9次方)
        // false  returnAfterSourceHandled 回调处理
        result = CFRunLoopRunSpecific(CFRunLoopGetCurrent(), kCFRunLoopDefaultMode, 1.0e10, false);
        CHECK_FOR_FORK();
        // 如果runloop 没有停止且没有结束继续循环
