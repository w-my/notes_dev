...
#if __arm64__
    MethodCacheIMP _imp;  // 函数、方法的内存地址
...
// 方法缓存 用散列表(哈希表)存储曾经调用过的方法 (空间换时间，提高效率)
struct cache_t {
    struct bucket_t *_buckets;  // 散列表  是一个数组 里面放的好多 bucket_t
    mask_t _mask;  // 散列表的长度
...
class list_array_tt {
 ...
 public:
    ...
    //addedLists : (方法、协议或者属性二维数组) [[method_t,method_t],[method_t,method_t]]
    // addedCount : 要追加的数组的长度
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        if (hasArray()) {
            ...
            // oldCount  类中原先的方法、属性或协议列表数组长度
            // newCount  追加新数组后的长度 (扩容)
            uint32_t oldCount = array()->count;
            uint32_t newCount = oldCount + addedCount; // 以前的长度 + 传进来的长度
            setArray((array_t *)realloc(array(), array_t::byteSize(newCount)));
            array()->count = newCount;
            
            // array()->lists  原来的方法列表
            // array()->lists + addedCount 指针移动，将原来数据往后移动 addedCount 长度，然后将分类类表拷贝到原来方法列表的位置
            memmove(array()->lists + addedCount, array()->lists, 
                    oldCount * sizeof(array()->lists[0]));
            
            // addedLists 所有分类的方法、属性或者协议列表
            // 将 addedLists 拷贝到原来的方法列表的位置，原来的方法列表往后移动 
            // 验证以前结论: 同样的方法 会优先使用分类的方法
            // 这样就导致分类的方法在最前面，原来类的方法在后面（验证了为什么会优先调用分类的方法）
...
// 每一个 method_list_t/property_list_t/protocol_list_t 但是一个独立的，可以是类的类方法列表，也可以是分类的方法列表
// 之所以定义为二维数组,可以方便的增加和修改方法、属性、协议列表，不影响其他的类相关信息
// 一开始类型相关信息(属性、方法、成员变量等)都是在 class_ro_t 内，在程序运行时，会把 class_ro_t 和 分类的信息合并放到 class_rw_t 中。
// 可以在 obj-runtime-new.mm realizeClass 源码实现
// method_list_t 是一维数组 内部装的都是 method_t
// method_t 源码 objc-runtime-new.h   struct method_t
struct class_rw_t {
    // Be warned that Symbolication knows the layout of this structure.
    uint32_t flags;
    uint32_t version;

    const class_ro_t *ro;  // 类的初始化内容 不包括分类的内容
...
// objc-cache.mm    bucket_t * cache_t::find(cache_key_t k, id receiver)
struct objc_class : objc_object {
    // Class ISA;
    Class superclass;
    cache_t cache;             // formerly cache pointer and vtable  方法缓存
    class_data_bits_t bits;    // class_rw_t * plus custom rr/alloc flags  用户获取具体的类信息

    class_rw_t *data() {   //获取 Class、Meta-Class对象的内存地址信息
        return bits.data();
    }
