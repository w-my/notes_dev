...
static inline mask_t cache_next(mask_t i, mask_t mask) {
    return i ? i-1 : mask;  //直接返回 i-1 如果减到 0 就直接返回 mask
}
...
bucket_t * cache_t::find(cache_key_t k, id receiver)
{
    ...
    bucket_t *b = buckets();
    mask_t m = mask();
    mask_t begin = cache_hash(k, m);  // k ： SEL   m：mask  begin 就是得到的索引
    mask_t i = begin;
    do {  // 如果找到 就直接返回
        if (b[i].key() == 0  ||  b[i].key() == k) {
            return &b[i]; // 返回 bucket_t 的地址
        }
    } while ((i = cache_next(i, m)) != begin);  // 没找到 调用 cache_next（i-1）
...
// buckets 数组不够用的时候会去扩容
void cache_t::expand()
{
    cacheUpdateLock.assertLocked();
    
    uint32_t oldCapacity = capacity();
    uint32_t newCapacity = oldCapacity ? oldCapacity*2 : INIT_CACHE_SIZE;  // 扩容2倍
