...
1:	cmp	p9, p1			// if (bucket->sel != _cmd)
	b.ne	2f			//     scan more
	CacheHit $0			// call or return imp  缓存中找到了就直接调用或者返回方法地址
	
2:	// not hit: p12 = not-hit bucket   // 没有命中 走 CheckMiss
...
// 下面的伪代码
//void _objc_msgSend(id recevier, SEL selector) {
//    if(recevier == nil) return;
//
//    //查找缓存
...
//}
    ...
    // p0 寄存器：消息接收者 reciver ，
    // 首先判断消息接受者是否为空 或 为tagged pointer,为空直接返回
	cmp	p0, #0			// nil check and tagged pointer check
#if SUPPORT_TAGGED_POINTERS
// b : 跳转  le : <= 0
// b.le    LNilOrTagged  条件成立的话 跳转 LNilOrTagged
	b.le	LNilOrTagged		//  (MSB tagged pointer looks negative)
#else
	b.eq	LReturnZero  // 成立直接返回
#endif
	ldr	p13, [x0]		// p13 = isa
...
.macro MethodTableLookup
	...
	mov	x2, x16
	bl	__class_lookupMethodAndLoadCache3  // 最终调用这个方法，跳出汇编到C++
...
.endmacro
	...
	MethodTableLookup  // 调用 MethodTableLookup
...
*   method caches. 实际上是函数指针存储在方法缓存中
*
********************************************************************/
