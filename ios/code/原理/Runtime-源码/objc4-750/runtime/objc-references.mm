...
// objc_getAssociatedObject 底层实现
id _object_get_associative_reference(id object, void *key) {
    id value = nil;  //关联对象
    uintptr_t policy = OBJC_ASSOCIATION_ASSIGN;
    // 大括号中保证安全释放
    {
        // 初始化AssociationsManager管理类
        AssociationsManager manager;
        
        /*
         C++中引用的知识点，对这一块不太了解的同学可以自行查阅
          int a = 10;
          int &b = a;
          b变量只是a的别名，两者访问的是同一块内存空间

         // manager.associations() 返回全局哈希键值对表的引用
...
// 设置关联对象
// objc_setAssociatedObject 底层实现
// valure 传入 nil 相当于删除一个关联关系
void _object_set_associative_reference(id object, void *key, id value, uintptr_t policy) {
    ...
    {
        //关联对象管理类，C++ 实现的一个类  (主要管理的就是 AssociationsHashMap 全局容器)
        AssociationsManager manager;
        //获取其维护的一个Hashmap 可以理解为是一个字典
        //是一个全局容器
        AssociationsHashMap &associations(manager.associations());
        //DISGUISE 按位取反作为指针变量的一个key或者一个标识
        disguised_ptr_t disguised_object = DISGUISE(object);
