...
// 类对象 元类对象 类型都是 Class
// class 和 mate-class 的底层都是 objc_class 结构体指针 
...
#define ISA_MASK        0x0000000ffffffff8ULL
    ...
    struct {
        ...
        // nonpointer 1 代表是优化过的 使用位域存储信息  0 代表是普通的指针
        uintptr_t nonpointer        : 1;                                         \
        // 是否有设置过关联对象，如果没有，释放时会更快
        uintptr_t has_assoc         : 1;                                         \
        // 是否有C++的析构函数（.cxx_destruct），如果没有，释放时会更快
        // 析构函数 就是用来释放内存的函数 类似 delloc
        uintptr_t has_cxx_dtor      : 1;                                         \
        // 存储着Class、Meta-Class对象的内存地址信息  (类对象后三位地址都是0)
