...
objc_object::clearDeallocating()
{
    ...
    // 优化过的指针 (目前使用的都是优化过的指针)
    // 判断是否使用了sideTable，使用的原因是因为做了weak引用（isa.weakly_referenced ） 或 使用了sideTable的辅助引用计数（isa.has_sidetable_rc）。符合这两种情况之一，则进入慢析构
    else if (slowpath(isa.weakly_referenced  ||  isa.has_sidetable_rc)) {  // 优化过的指针
...
objc_object::rootDealloc()
{
    ...
    // nonpointer = 0 代表普通的指针  = 1 代表过的(使用位域存储更多的信息)    详见 isa 指针
    // weakly_referenced 是否有被弱引用指向过，如果没有，释放会更快
    // has_assoc 是否设置过关联对象，如果有，释放时会更快
    // has_cxx_dtor 是否有C++析构函数 ，如果没有释放会更快
    // has_sidetable_rc 引用计数器是否过大无法存储在 isa 中，如果为 1 ，那么引用计数会存储在一个 SideTable 的类的属性中
    ...
    {
        assert(!sidetable_present());
        // 如果是优化过的指针，且满足上面的条件就直接 free
        free(this);
    } 
    else {  // 其他情况
        object_dispose((id)this); // object_dispose 慢慢释放
    }
}
...
objc_object::rootRetainCount()
{
    ...
    ClearExclusive(&isa.bits);
    if (bits.nonpointer) { // 如果是优化过的 isa
        uintptr_t rc = 1 + bits.extra_rc;  // +1 返回
        if (bits.has_sidetable_rc) {  // 如果 has_sidetable_rc = 1 ，表明不是存储在 isa 中，是存储在 SideTable 中
