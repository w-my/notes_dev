...
// objc_object 为 key 的 hash 表，value OC 对象的引用计数
// 当OC对象的引用计数变为0时，会自动将相关的信息从hash表中剔除
...
//在 64bit 中，引用计数可以直接存储在优化过的isa指针中，也可能(isa_t的引用计数溢出的情况)存储在 SideTable 类中 (未开启 isa 优化直接存储在 SideTable 中 )
//是对 weak_table_t 表的再次封装，避免对 weak_table_t 直接操作
struct SideTable {
    spinlock_t slock; // 自旋锁
    RefcountMap refcnts;  //refcnts 是一个存放着对象引用计数的散列表
    weak_table_t weak_table;  // 弱引用散列表
    /*
     SideTables 有 64个 SideTable节点，一般不会只有 64个对象，因此多个对象会同用一个SideTable 节点，这样 weak_table 会存储多个对象的弱引用信息，因此在SideTable 中，又会通过 weak_table 作为hash 表再次分散存储每一个对象的弱引用信息
     */
...
#pragma mark SideTables 全局的 hash 数组
// SideTables 存储了 SideTable 类型的数据，长度时64
// 本质是一个 StripedMap

/*
 我们不能使用c++静态初始化器来初始化SideTables，因为在c++初始化器运行之前，
 libc会调用我们。我们也不想要全局的指针指向这个结构体，因为有额外的间接。
 用艰苦的方式去做。
 */
...
#pragma mark 为某个对象初始化一个弱(weak)指针
/** 为某个对象初始化一个弱(weak)指针
 ...
 *
 * @param location Address of __weak ptr.  //  weak 指针地址 ( &weakPtr)  是指针的地址
 * @param newObj Object ptr.   // 指向的对象 （o）
 */
...
#   define POOL_BOUNDARY nil  //是一个边界对象 nil,之前的源代码变量名是 POOL_SENTINEL哨兵对象,用来区别每个page即每个 AutoreleasePoolPage 边界
    ...
    id *next;  //指向新加入的autorelease对象下一个位置
    pthread_t const thread;  //page当前所在的线程
...
objc_object::sidetable_getExtraRC_nolock()
{
    ...
    // 根据key this 取出 value
    SideTable& table = SideTables()[this];
    // 根据key this 在引用计数散列表refcnts中取出 it
    // 引用计数表中用迭代器查找当前实例对应的键值对，获取引用计数值
    RefcountMap::iterator it = table.refcnts.find(this);
...
#pragma mark objc_autoreleasePoolPop
void
