...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    // 属性数组 [[property_t,property_t],[property_t,property_t]]
    property_list_t **proplists = (property_list_t **)
        malloc(cats->count * sizeof(*proplists));
    
    // 协议数组 [[protocol_t,protocol_t],[protocol_t,protocol_t]]
    ...
    int i = cats->count;
    bool fromBundle = NO;
    while (i--) {  // i-- 最后参与编译的分类在最方法列表的最前面(验证了最后参与编译的方法最优先调用)
        // 取出 某个分类
    ...
    // class_rw_t  得到类对象里面的数据
    auto rw = cls->data();

    prepareMethodLists(cls, mlists, mcount, NO, fromBundle);
    
    // 将分类的对象方法追加到类对象的方法列表中
...
load_images(const char *path __unused, const struct mach_header *mh)
{
    ...
    // Discover load methods
    {
        ...
        //准备 load 方法, loadable_classes 、loadable_categories 数组的建立
...
#pragma mark - Discover categories
    // Discover categories.   发现分类  处理分类
    for (EACH_HEADER) {
        category_t **catlist = 
...
static void schedule_class_load(Class cls)
{
    ...
    // 将 cls 加到 loadable_classes 数组的后面
...
void prepare_load_methods(const headerType *mhdr)
{
    ...
    for (i = 0; i < count; i++) {
        // prepare_load_method 类的 load 方法列表
    ...
    for (i = 0; i < count; i++) {
        ...
        // add_category_to_loadable_list 分类的 +load 方法列表
...
* already tried that.
**********************************************************************/
// 从汇编调到这里，不会在查找缓存，汇编已经查找过
IMP _class_lookupMethodAndLoadCache3(id obj, SEL sel, Class cls)
{
    // obj 消息接受者  cls 类名 sel 方法
    // NO 是因为已经在汇编中查找过缓存，没有查找到
...
/***********************************************************************
* lookUpImpOrForward.
* The standard IMP lookup.  标准的 IMP 查找。
...
{
    IMP imp = nil;
    bool triedResolver = NO;  // 标识是否已经动态解析
 ...
 retry:    
    ...
    // Try superclass caches and method lists.
    {
        ...
        for (Class curClass = cls->superclass;
             // RootClass(NSObject) 的 superClass == nil
             curClass != nil;
             // for 循环一直往上找
...
#pragma mark 动态方法解析
    ...
    // 如果父类也没有找到  就走一次动态方法解析
    // resolver 解析器
    // No implementation found. Try method resolver once.

    
    // 尝试动态解析 以前未动态解析过就走动态解析
...
#pragma mark objc_destructInstance 析构函数
/***********************************************************************
...
void *objc_destructInstance(id obj) 
{
    if (obj) {
        ...
        if (cxx) object_cxxDestruct(obj);  // 调用 C++ 析构函数清除成员变量
        if (assoc) _object_remove_assocations(obj);  // 移除关联对象，并将其自身从Association Manager的map中移除
